...
CREATE INDEX IF NOT EXISTS idx_shift_closes_business ON public.shift_closes(business_id);
CREATE INDEX IF NOT EXISTS idx_shift_closes_date     ON public.shift_closes(business_id, shift_date);

ALTER TABLE public.shift_closes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "owner can manage shift_closes" ON public.shift_closes;
CREATE POLICY "owner can manage shift_closes"
  ON public.shift_closes
  FOR ALL
